  ...
  function ($, can) {
    return can.Control.extend({
      ...
      hoverOver: function (element, event) {
        this.element.find('.btn-prev').show().stop(true, false).animate({ left:  0, opacity: 1 }, 500);
        this.element.find('.btn-next').show().stop(true, false).animate({ right: 0, opacity: 1 }, 500);
        clearInterval(this.options.silderTimer);
      },

      hoverOut: function (element, event) {
        var that = this;
        this.element.find('.btn-prev').stop(true, false).animate({ left:  '-50px', opacity: 0 }, 500, function(){ that.element.find('.slider .btn-prev').hide() });
        this.element.find('.btn-next').stop(true, false).animate({ right: '-50px', opacity: 0 }, 500, function(){ that.element.find('.slider .btn-prev').hide() });
