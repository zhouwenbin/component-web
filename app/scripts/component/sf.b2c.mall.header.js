...
// @description 申明命名空间
sf.util.namespace('b2c.mall.header');

/**
 * @class sf.b2c.mall.header
 * @author Michael.Lee (leewind19841209@gmail.com)
 * @description 加载商品列表数据并且进行显示
 */
sf.b2c.mall.header = can.Control.extend({
  ...
  init: function(element, options) {
    if($.cookie('ct') == 1){
      this.data = new can.Map(_.extend(this.defaults.login, {user: null}));
    }else{
      this.data = new can.Map(_.extend(this.defaults.nologin, {user: null}));
    }
