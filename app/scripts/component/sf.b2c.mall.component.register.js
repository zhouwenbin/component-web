...
define(
  ...
  function($, $cookie, can, md5, _, store, placeholders, SFApiUserDownSmsCode, SFApiUserMobileRegister, SFApiUserSendActivateMail, SFBizConf, SFFn, SFMessage, SFReceivePro, SFCheckUserExist) {
    ...
    var DEFAULT_ACTIVATE_ERROR_MAP = {
      '1000020': '邮箱地址已存在，<a href="i.login.html">立即登录</a>',
      '1000050': '邮箱地址错误',
      '1000070': '参数错误',
      '1000100': '验证码错误',
      '1000160': '邮件请求频繁'
    ...
    return can.Control.extend({
      ...
      '{can.route} change': function() {
        // var tag = can.route.attr('tag') || DEFAULT_FILLINFO_TAG;
        if (this.options.tag == "success") {
          return false;
        }

        this.options.tag = can.route.attr('tag') || DEFAULT_FILLINFO_TAG;

        this.render.call(this, this.options.tag, this.data);
      },
      ...
      '#mobile-register-btn click': function($element, event) {
        ...
        if (this.checkMobile.call(this, mobile) && this.checkCode(code) && this.checkPassword(password, '#password-error')) {
          ...
          this.component.mobileRegister
            ...
            .then(function(data) {
              // 必须在登录后才能调用，所以放到注册接口后面调用
              var receivePro = new SFReceivePro({
                "channel": "B2C",
                "event": "REGISTER_USER_SUCCESS"
              });
              return receivePro.sendRequest();
            })
      ...
      '#mail-register-btn click': function($element, event) {
        ...
        if (this.checkEmail.call(this, email) && this.checkMailCode.call(this, code)) {
          ...
          this.component.activateMail.sendRequest()
            .done(function(data) {
              if (data.value) {
                window.location.href = SFBizConf.setting.link.register + '?' + $.param({
                  mailId: email
                }) + '#!&tag=confirminfo';
